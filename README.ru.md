...
<div align="right"><i><a href="./README.en.md">English</a> · <b>Русский</b> · <a href="./README.de.md">Deutsch</a> · <a href="./README.ko.md">한국어</a> · <a href="./README.md">简体中文</a> · <a href="./README.zh-TW.md">繁體中文</a> · <a href="./README.ja.md">日本語</a> · <a href="./README.tr.md">Türkçe</a></i></div>

> ⚠️ Это форк CloudCLI UI, изменённый в соответствии с личными предпочтениями, а не официальная upstream-версия.
> Основные текущие изменения: реальное потоковое отображение процесса размышления и ответа, развёрнутые по умолчанию размышления, умная автопрокрутка при потоковой выдаче, мгновенная обратная связь по статусу Processing и звуку завершения, оценка длительности и токенов размышлений в реальном времени, а также прямая отправка мультимодальных изображений через Claude provider.
> В будущем будем стараться сохранять синхронизацию с upstream ([siteboon/claudecodeui](https://github.com/siteboon/claudecodeui)).
> **README по умолчанию на китайском (упрощённом).**
...
- **Настоящий потоковый вывод** - процесс размышления и текст ответа рендерятся токен за токеном в реальном времени, размышления развёрнуты по умолчанию с живой оценкой длительности и токенов
- **Более быстрая обратная связь статуса** - баннер Processing и звук завершения срабатывают сразу при окончании потоковой выдачи, без ожидания финального события SDK
- **Сообщения с изображениями** - отправляйте изображения напрямую через Claude provider, без промежуточных локальных временных файлов
- **Умная автопрокрутка** - автоматически удерживает нижнюю часть чата на экране во время потоковой выдачи
